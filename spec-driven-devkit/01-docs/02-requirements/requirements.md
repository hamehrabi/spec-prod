...
| REQ-F-033 | A developer must be able to run the intake at an **express depth** that asks at most **two questions per round instead of four** and writes thinner files — over the same eight rounds — so that a small or exploratory project is not forced to carry full depth. | Must |
...
| AC-031 | REQ-F-033 | **Given** a developer who selects express depth, **When** intake completes, **Then** the workspace is thinner and ran **the same eight rounds** as the default depth, each asking at most two questions rather than four — and every structural rule still holds: identifiers resolve, blueprint links resolve, and unknowns are `[TODO]` with matching open questions. |
