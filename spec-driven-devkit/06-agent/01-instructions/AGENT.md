...
|---|---|---|
| 2026-08-04 | **BUG-007.** A full intake ran fifteen minutes and wrote nothing. No step was slow; the instruction set had simply grown to fourteen modules and nothing said when to read them, so a thorough agent read everything before acting. | **When adding a module to an instruction set, say WHEN it is opened, not just what it contains.** And when a requirement is about *time to first output*, it needs an instruction enforcing it — REQ-NF-001 was correct, unenforced, and carried as "unmet" through six tasks while getting quietly worse. A requirement with no instruction behind it is a hope. |
| 2026-08-04 | **BUG-004.** Told to compare 79 checksums with no method offered and no prohibition stated, the intake wrote two shell scripts into a developer's repository — outside `spec/`, before the preamble. Improvising was rational given what the instruction said. | **When a step needs a capability the kit does not have, say so and stop. Never build the capability out of files in someone else's repository.** And when writing an instruction: if it demands an outcome, it must also name the means and say what to do when the means is absent. An instruction that specifies only the outcome will be satisfied by whatever the reader can reach for. |
| 2026-08-04 | **BUG-003.** Contract C2 guarantees the worked example is *always last*, and ADR-003 deletes to end of file on the strength of it. Three blueprints had broken the guarantee; every C2 test asserted the **generated** side, none the **guaranteeing** side. | **A contract needs a test on the side that promises, not only the side that relies.** When an algorithm's correctness rests on a stated property of its input, assert the property on the input. Otherwise a library defect can only surface as a mysterious output defect — and here it would have surfaced as a section silently missing from a structurally valid file. |
