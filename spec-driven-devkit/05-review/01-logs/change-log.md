...
| 2026-08-03 | **TASK-001 implemented** — the plugin installs, registers one command, prints the preamble, exits | Added | The thinnest vertical slice. Proves the whole delivery path before anything can write to a repository | `plugin/**` (4 files), DD-014, DD-015 | Kit author |
| 2026-08-03 | **TASK-002 implemented** — FF-001, FF-002, FF-009 wired into a merge gate | Added | Added second, deliberately, so ADR-002 is enforced from the first commit that could break it | `ci/**`, `tests/**`, `.github/workflows/gate.yml`, DD-016…019 | Kit author |
| 2026-08-03 | **The repository was made public** | Changed | Branch protection is unavailable on a private repo at this plan, so the gate could report but never block. Public is the only $0 option that lets a fitness function actually fail a build | DD-017; **Q-007 escalated**; Q-022 closed first, by scan | Kit author |
| 2026-08-03 | **Plugin payload placed at `plugin/`, outside the specification workspace** | Changed | The specification contradicted itself — `04-src/` in two files, outside `spec/` in a third. Resolved so the payload is one subtree FF-009 can name, and the kit's own 113 spec files never ship | DD-015, DD-016; `04-src/README.md` now describes a layout that lives elsewhere | Kit author |
| 2026-08-03 | **A second command was pre-rejected in code, not only in prose** | **Rejected** | FF-001 now fails the build on a second user-invocable command *or* a second orchestration path. The rule existed since ADR-001; until this it was a sentence | FF-001, UTEST-013, BUG-001 | Kit author |
