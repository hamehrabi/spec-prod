...
| EV-001, all 8 rounds, express — **third e2e verification** | 2026-08-08 | **Duplicates 0 for the third consecutive run; BUG-048 VERIFIED** (the produced checklist carries the declaration; check 5 no longer reports its slot). Exposed two checker gaps (backticked example ids, the drafted-release date stub — fixed as UTEST-096/097) and two more BUG-034b-class forms (risk-storming, traceability-review) plus AGENT.md's seed row — **BUG-049/BUG-050, payload-fixed, unverified until a future run**. Fixture NOT swapped: the committed baseline is the cleaner sample, and nothing this run verified needs the swap | 2 legs, ~57 min real (+26 min machine suspend mid-leg-1); host again ended one step early, resume banked it | $23.16 + $4.61 | Recorded, not accepted |
