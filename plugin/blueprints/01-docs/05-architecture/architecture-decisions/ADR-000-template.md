...
Context:
The first version must support projects, tasks, roles, and team collaboration. The system
needs clear organization, but the team does not need separate deployments for each
feature yet.

Options Considered:
1. Simple monolith   - fast, but risks mixing responsibilities.
2. Modular monolith  - one deployment with clear internal boundaries.
3. Microservices     - flexible, but too complex for the first version.

Decision:
Use a modular monolith.

Reason:
This gives the project enough structure for long-term growth while keeping development,
testing, and deployment simple.
