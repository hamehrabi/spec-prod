...
## The process, actually walked

| Stage | Artifact produced | Quality gate | Passed? |
|---|---|---|---|
| Idea intake | `intent.md` — TeamTask/ProjectBoard | Problem, users, constraints, non-goals clear | ✅ 2026-03-01 |
| Requirements | 11 requirements + acceptance criteria | Each testable or rejectable | ✅ 2026-03-04 |
| Design | Tech spec + ADR-001…005 | Architecture explicit and justified | ✅ 2026-03-06 |
| Tasking | 16 tasks, P0–P3, dependency map | Each small, bounded, reviewable | ✅ 2026-03-08 |
| Testing | 61 tests across 6 levels | Behavior, failures, edge cases covered | ✅ 2026-03-10 |
| Implementation | 14 of 16 tasks agent-assisted | Code matches specs, no unrelated changes | ⚠ 4 scope findings |
| Review | 5 review passes, 11 findings | Checked against req, arch, security, tests | ✅ 1 release held |
| Release and learning | v1.0.0 + 6 feedback items + 5 drift items | Production feedback became spec updates | ✅ CHG-003…005 |

**Where the gate actually fired:** v1.0-rc1 was held because SEC-A-002 had a spec and a
test but no code. The Review gate caught what 61 green tests could not.

## Template library improvements this cycle

| Recurring problem | Template improved | Change made | Date |
|---|---|---|---|
| Agent edits files outside the task | `TASK-001.md` | "Expected files" and "Do not change" became required fields | 2026-03-28 |
| Allow path built, deny path forgotten | `AGENT.md` | v1.2: implement and test the denial path in the same task | 2026-04-01 |
| Null assumptions causing 500s | `AGENT.md` | v1.3: handle the null path; confirm the spec states the behavior | 2026-04-04 |
| Ambiguity discovered mid-build | `intent.md` prompts | "Clarify before building" run on every new requirement | 2026-04-10 |

**Effect:** scope-creep review findings dropped from 4 in March to 1 in April.

## Repeatable system checklist — April

| Area | Question | Ready? |
|---|---|---|
| Process | Can you explain the path from idea to production review? | **Yes** |
| Documentation | Can a new human or AI agent find the current source of truth? | **Yes** — source-of-truth order in AGENT.md |
| Versioning | Are requirement and spec changes named, dated, explained? | **Yes** — CHG-001…005 |
| Templates | Do projects reuse proven briefs, specs, test plans, checklists? | **Yes** — 4 improved this cycle |
| Agent rules | Do agents receive clear constraints and completion rules? | **Yes** — AGENT v1.3 |
| Traceability | Can each feature be traced req → task → test → code → review → release? | **Yes** — 11/11 rows complete |
| Quality metrics | Do you measure defects, rework, findings, drift, stability? | **Yes** — see engineering-quality-review |
| Adoption | Does the workflow reduce confusion and improve evidence? | **Yes** — clarifying questions 14 → 6 |

## Final guidance, applied

| # | Guidance | How ProjectBoard did it |
|---|---|---|
| 1 | Begin with engineering intent | `intent.md` written before any code request |
| 2 | Turn intent into testable requirements | 11 requirements, all with acceptance criteria |
| 3 | Write tech specs before decisions scatter | Tech spec + 5 ADRs before TASK-005 |
| 4 | Give agents bounded tasks | 16 tasks with allowed-files lists |
| 5 | Plan tests before generation | 61 tests written from acceptance criteria |
| 6 | Review against req, arch, security, maintainability | 5 passes, 11 findings, 1 release held |
| 7 | Deploy with rollback and monitoring | Tag v0.9.3, 5 rollback triggers, 8 signals |
| 8 | Treat production feedback as spec lifecycle | FB-001…006 → CHG-003…005 |
| 9 | Keep improving templates and rules | 4 template changes, 3 agent-rule versions |

> **The one that mattered most:** #9. Every other step was executed correctly in March and
> the project still produced 4 scope-creep findings and 2 incidents. Improving the
> *templates* — not the people, not the prompts — is what changed April's numbers.
