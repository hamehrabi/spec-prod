...
|---|---|---|---|
| Product handoff | PM defines goal, users, acceptance criteria, non-goals. | None yet. | PRD section approved. |
| Technical plan | Developer defines endpoint, permissions, data sources, scheduling rules. | Agent is asked to **critique the design for gaps**. | Technical spec updated. |
| Task brief | Developer creates one task: generate API tests from acceptance criteria. | Agent drafts **test cases only**. | Tests reviewed before code. |
| Implementation | Developer assigns one bounded implementation task. | Agent generates draft code and notes assumptions. | Code reviewed against requirements and architecture. |
| Feedback | Reviewer finds a rule the spec missed. | Agent updates tests and code **after** the spec update. | Regression test added. |
| Spec update | Team updates the rule and traceability matrix. | Agent receives a **refreshed context pack** for the next task. | Source of truth matches implementation. |
