// UTEST-052…051 — the seven validation checks that judged nothing, each pinned in BOTH
...
// --- UTEST-052: check 2, a citation is not a definition ---------------------------------------

const requirements = (text = 'A cook can save a recipe with its ingredients.') =>
  `# Requirements\n\n| ID | Requirement | Priority |\n|---|---|---|\n| REQ-F-001 | ${text} | Must |\n${back()}`

test('UTEST-052: a filled traceability matrix is not a second definition of every requirement', () => {
...
test('UTEST-052: a real second definition, in a second file, still fails', () => {
...
test('UTEST-052: the rule is written down, with the shapes it reads as citations', () => {
...
// --- UTEST-053: check 6, a citation is not a distance -------------------------------------------

const openQuestions = (...rows) =>
  '# Open Questions\n\n| ID | Question | Why it matters | Owner | Status |\n|---|---|---|---|---|\n' +
  rows.map((r) => `| ${r} |\n`).join('')

test('UTEST-053: a Q-### row near an unrelated [TODO] does not pair with it', () => {
...
test('UTEST-053: the same marker, ~520 characters away, used to flip the verdict — now it does not', () => {
...
test('UTEST-053: a citation inside the marker pairs — and only to a question that exists', () => {
  const cited = {
...
test('UTEST-053: the written rule says what "beside itself" means', () => {
...
// --- UTEST-054: check 7, touched and not decided ------------------------------------------------

const rateLimits = (loginRow) =>
  `# Runtime\n\n| Endpoint | Limit | Window | Keyed by | On breach | Notes |\n|---|---|---|---|---|---|\n${loginRow}\n${back()}`

test('UTEST-054: a row that names its subject and decides nothing FAILS', () => {
...
test('UTEST-054: a row with a single unfilled cell is sparse, not undecided', () => {
...
test('UTEST-054: a row with every cell blank still fails, and is not read as a delimiter', () => {
...
test('UTEST-054: the written rule names the row it is about', () => {
...
// --- UTEST-055: check 8, a deny test per rule ---------------------------------------------------

const roles = (cannot) => `| Role | Can do | Cannot do |\n|---|---|---|\n| Cook | Save their own recipes | ${cannot} |\n\n`
const roleRules = (rows) => `| ID | Role requirement |\n|---|---|\n${rows}\n\n`

test('UTEST-055: permission rules with no deny test FAIL, however much prose surrounds them', () => {
...
test('UTEST-055: a denial rule with no test citing it is named', () => {
  const ws = {
    'spec/x.md':
...
test('UTEST-055: the rule, its denial, and the criterion that cites it — passes', () => {
  const ws = {
    'spec/x.md':
      '# Roles\n\n' + roles('Read another account’s records') +
      roleRules(
...
test('UTEST-055: a workspace with no permission rules is NOT RUN, never passed', () => {
  const r = CHECKS[8].run({ 'spec/x.md': `# X\n\nNothing here declares a role. A list cannot be empty.\n${back()}` })
  assert.equal(r.state, 'not-run')
})

test('UTEST-055: the written rule says where the denial words may be read', () => {
...
test('UTEST-056: one FF-### elsewhere in the workspace no longer governs three drivers', () => {
...
test('UTEST-056: every driver naming its own fitness function passes', () => {
  const ws = {
...
test('UTEST-056: the written rule says the evidence is in the row', () => {
...
// --- UTEST-057: check 10, the walk runs twice ---------------------------------------------------
...
test('UTEST-057: before the entry point exists, the walk permits writing it and refuses success', () => {
...
test('UTEST-057: the second walk, with the entry point written, may claim success', () => {
  const ws = finished()
...
test('UTEST-057: any OTHER not-run still blocks the entry point', () => {
...
test('UTEST-057: a failing check blocks the entry point too', () => {
  const ws = finished()
...
test('UTEST-057: the ordering is written down, both passes and their questions', () => {
...
// --- UTEST-058: check 10 reads the paths a map actually writes ----------------------------------

test('UTEST-058: a Start-here table of backticked paths is read, and a broken one fails', () => {
...
test('UTEST-058: a Start-here section that resolves no path at all fails', () => {
  const r = CHECKS[10].run({ 'spec/CLAUDE.md': '# Map\n\n## Start here\n\nRead the requirements, then the tasks.\n' })
  assert.equal(r.state, 'failed')
  assert.match(r.detail.join(' '), /resolves no path/)
})

// --- UTEST-059: check 13, the skip row as the instructions teach it ------------------------------
...
test('UTEST-059: the skip row exactly as coverage.md shows it satisfies the check', () => {
...
test('UTEST-059: the full manifest path is accepted too, and the exclusion is named out loud', () => {
...
test('UTEST-059: a skip that resolves to nothing is a finding, not a silent pass', () => {
...
test('UTEST-059: an ambiguous filename asks for the path rather than guessing', () => {
...
test('UTEST-059: a blueprint neither filled nor skipped still fails, named by path', () => {
  const r = CHECKS[13].run(withSkip(''), MANIFEST)
  assert.equal(r.state, 'failed')
  assert.match(r.detail.join(' '), /frontend-component-spec\.md/)
})

test('UTEST-059: given the manifest itself, only the blueprints are required', () => {
...
test('UTEST-059: the two forms and the exclusion are written down', () => {
