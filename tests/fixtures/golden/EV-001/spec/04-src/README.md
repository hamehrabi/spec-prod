...
The layout below mirrors the modular monolith (ADR-001) so an AI agent does not create
...
04-src/
  ...
  domain/         # business logic — recipes, planning, shopping-list generation
...
|---|---|---|
| `ui/` | Screens, forms, display states, user actions. | Contain database queries or hidden business rules. |
| `api/` | Routes, request validation, response formatting. | Hide complex domain logic in route handlers. |
| `domain/` | Business rules and core decisions (list generation). | Depend directly on screen layout or format HTTP responses. |
...
- Use portable relational SQL only — no SQLite-only or Postgres-only features (ADR-002).
